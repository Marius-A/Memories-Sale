...
Plugin URI: http://aelia.co/shop/currency-switcher-woocommerce/
...
class WC_Aelia_CurrencySwitcher implements IWC_Aelia_CurrencySwitcher {
	...
	/**
	 * Converts a product's price in the currently selected currency.
	 *
	...
	protected function set_shipping_methods_hooks() {
		...
		if(version_compare($this->wc()->version, '2.6', '>=')) {
	...
	protected function set_refund_metadata($refund) {
		$refund_currency = $refund->get_order_currency();
		// Set refund currency, if it's missing
		if(empty($refund_currency)) {
			$parent_order_id = wp_get_post_parent_id($refund->id);
			...
			if(is_numeric($parent_order_id)) {
				$parent_order = new Aelia_Order($parent_order_id);
				$order_currency = $parent_order->get_order_currency();
			}
			else {
				$order_currency = get_woocommerce_currency();
			}
			...
			update_post_meta($refund->id, '_order_currency', $order_currency);
		}
	}
	...
	private function set_hooks() {
		...
		if(self::is_frontend()) {
			...
			// Add filters to alter price based on selected currency
	...
	private function save_user_selected_currency($selected_currency) {
		...
		Aelia_SessionManager::set_cookie(AELIA_CS_USER_CURRENCY,
																		 $selected_currency,
																		 time() + DAY_IN_SECONDS);
	}
	...
	public function get_selected_currency() {
		if(empty($this->selected_currency)) {
			...
			if(!self::settings()->debug_geolocation_currency_detection()) {
				...
				$this->selected_currency = coalesce(Aelia_SessionManager::get_cookie(AELIA_CS_USER_CURRENCY),
																						$user_currency);
			}
	...
	public function wc_coupon_loaded($coupon) {
		$this->currencyprices_manager()->set_coupon_amounts($coupon);
		return $coupon;
	}
	...
	protected function set_shipping_method_params_in_currency($shipping_method) {
		...
		if(empty($shipping_method->shipping_prices_in_currency)) {
			if(!empty($shipping_method->min_amount)) {
				$shipping_method->min_amount = $this->convert($shipping_method->min_amount,
																						 $this->settings_controller()->base_currency(),
	 ...
	 *
	 * @param array shipping_methods An array of Shipping Methods class names or
	 * object instances.
	 * @return array
	 * @deprecated since WooCommerce 2.6
	 */
	...
	public function woocommerce_loaded() {
		...
		if(empty($selected_currency) && !self::editing_order() && $this->currency_by_billing_country_enabled()) {
	...
	protected function register_common_admin_scripts() {
		...
		wp_register_script('wc-aelia-currency-switcher-product-edit',
											 AELIA_CS_PLUGIN_URL . '/js/admin/' . $js_path . '/wc-aelia-currency-switcher-product-edit.js',
	 ...
	 * @return bool
	 */
	public static function doing_ajax($only_if_frontend = true) {
	...
	public function get_billing_country() {
		...
		if(self::doing_ajax() &&
			...
			if(isset($_POST[AELIA_CS_ARG_ORDER_BILLING_COUNTRY])) {
				...
				Aelia_SessionManager::set_cookie(AELIA_CS_SESSION_BILLING_COUNTRY, $result);
			}
		}
		...
		if(empty($result)) {
			$result = Aelia_SessionManager::get_cookie(AELIA_CS_SESSION_BILLING_COUNTRY);
		}
